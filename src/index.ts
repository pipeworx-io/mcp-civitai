...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Civitai');
}
...
async function getModel(args: Record<string, unknown>): Promise<unknown> {
  ...
  // Same reasoning one line down: a caller passing a non-numeric id is a caller
  // mistake, not our defect.
  if (!Number.isFinite(id) || id <= 0) {
    return { error: 'user_error: provide a numeric Civitai model id, e.g. {"id": 4201}', id: args.id ?? null };
  }

  const raw = await cvGet(`/models/${id}`, {});
  // A model id Civitai does not carry is a NEGATIVE ANSWER about that id, not a
  // fault of ours. Untagged it booked blob4='error' — the class that means our
  // bug and the only one problem-tool ranking counts — so an external caller
  // walking ids put this pack on the broken list at a 9.5% "error" rate.
  // `not_found:` is the token the gateway classifier already strips.
  if ((raw as Record<string, unknown>).__notfound) {
    return {
      error: `not_found: Civitai has no model with id ${id}. Ids are Civitai's own and are not sequential — resolve one with search_models rather than guessing. This says nothing about whether the model existed and was removed.`,
      id,
      found: false,
    };
  }
